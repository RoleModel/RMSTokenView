...
- (void)setupConstraintsOnSummaryLabel:(UILabel *)summaryLabel {
    ...
    [self.tokenContentView addConstraint:[NSLayoutConstraint constraintWithItem:summaryLabel attribute:NSLayoutAttributeCenterY relatedBy:NSLayoutRelationEqual toItem:self.tokenContentView attribute:NSLayoutAttributeTop multiplier:1.0 constant:21]];
    [self addConstraintsWithFormat:@"H:|-10-[view]-12-|" withView:summaryLabel toView:self.tokenContentView];
    [self addConstraintsWithFormat:@"V:|-5-[view]" withView:summaryLabel toView:self.tokenContentView];
    [summaryLabel setContentCompressionResistancePriority:UILayoutPriorityDefaultLow forAxis:UILayoutConstraintAxisHorizontal];
}
...
- (void)updateConstraintsForTokenLines:(NSArray *)tokenLines andLineView:(UIView *)lineView withTextFieldFocus:(BOOL)textFieldHasFocus isSearching:(BOOL)isSearching {
    ...
    if (textFieldHasFocus) {
        [self.updatingConstraints addObjectsFromArray:[NSLayoutConstraint constraintsWithVisualFormat:@"V:[lineView(1)]|" options:(NSLayoutFormatOptions)0 metrics:nil views:NSDictionaryOfVariableBindings(lineView)]];
    } else {
        [self.updatingConstraints addObjectsFromArray:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-43-[lineView(1)]" options:(NSLayoutFormatOptions)0 metrics:nil views:NSDictionaryOfVariableBindings(lineView)]];
    }
